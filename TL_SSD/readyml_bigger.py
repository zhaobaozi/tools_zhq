...
images = yaml.load(open('/home/zhaohuaqing/Downloads/tl_ssd/DTLD_Labels/Berlin_all.yml','rb').read())
image_new='/home/zhaohuaqing/Downloads/tl_ssd/bigger_box_xml/'
#image_new='/home/zhaohuaqing/Downloads/tl_ssd/images8/'
root_path='/home/zhaohuaqing/Downloads/tl_ssd/'
labelpath='/home/zhaohuaqing/Downloads/tl_ssd/biger_box_labels_xml/'
#labelpath='/home/zhaohuaqing/Downloads/tl_ssd/labels8/'
folder = os.path.exists(image_new)
if not folder:
    os.makedirs("/home/zhaohuaqing/Downloads/tl_ssd/bigger_box_xml")
folder1 = os.path.exists(labelpath)
if not folder1:
    os.makedirs("/home/zhaohuaqing/Downloads/tl_ssd/biger_box_labels_xml")
context_1=0.25
context_2=0.10
        ...
        if classid[0]==str(1) and classid[4]!=str(3) and classid[5]!=str(8) and classid[5]!=str(9):
            if label['width']*label['height']>200:
                ...
                if x1 == 0 or y1 == 0 or x2 == w or y2 == h:
                    ...
                    w_new_11=re_x2-re_x1
                    h_new_11=re_y2-re_y1
                    re_x1_n = int(re_x1-context_1*w_new_11)
                    re_y1_n = int(re_y1-context_2*h_new_11)
                    re_x2_n = int(re_x2+context_1*w_new_11)
                    re_y2_n = int(re_y2+context_2*h_new_11)
                    if re_x1_n <= 0: re_x1_n = 0
                    if re_x2_n>= w_new: re_x2_n = w_new
                    if re_y1_n <= 0: re_y1_n = 0
                    if re_y2_n >= h_new: re_y2_n = h_new
                    #cv2.rectangle(imgcrop, (re_x1_n,re_y1_n), (re_x2_n, re_y2_n), (0, 255, 0), 1)
                    ...
                    with open(new_labeltxt, 'a+') as f:
                        str1 =list1[:-5] + '-' + str(j) +'-'+ str(count)+'.jpg' + ' ' + classname[int(classid[4])] + ' ' + str(
                            re_x1_n) + ' ' + str(re_y1_n) + ' ' + str(re_x2_n) + ' ' + str(re_y2_n) + '\n'
                        f.write(str1)
                else:
                    ...
                    while(count<count_d):
                            ...
                            if IOU>0.8:
                                ...
                                if re_y1_n <= 0: re_y1_n = 0
                                if re_y2_n >= h_new: re_y2_n = h_new
                                #cv2.rectangle(imgcrop_2, (re_x1_n,re_y1_n), (re_x2_n, re_y2_n), (0, 255, 0), 1)
                                cv2.imwrite(image_new + list1[:-5] + '-' + str(j) +'-'+str(count)+ '.jpg', imgcrop_2)
                                with open(new_labeltxt, 'a+') as f:
                                    str1 =list1[:-5] + '-' + str(j) + '-'+str(count)+'.jpg' + ' ' + classname[int(classid[4])] + ' ' + str(re_x1_n) + ' ' + str(re_y1_n) + ' ' + str(re_x2_n) + ' ' + str(re_y2_n) + '\n'
                                    f.write(str1)
                ...
                if j>=2:
                    ...
                    for c in range(1,sumcount_0+1):
                        ...
                        if IOU >= 0.7:
                            ...
                            w_new_22 = str3x2 - str3x1
                            h_new_22 = str3y2-str3y1
                            re_x1_n1 = int(str3x1 - context_1 * w_new_22)
                            re_y1_n1 = int(str3y1 - context_2* h_new_22)
                            re_x2_n1 = int(str3x2 + context_1 * w_new_22)
                            re_y2_n1 = int(str3y2 + context_2 * h_new_22)
                            if re_x1_n1 <= 0: re_x1_n1 = 0
                            if re_x2_n1 >= w_crop: re_x2_n1 = w_crop
                            if re_y1_n1 <= 0: re_y1_n1 = 0
                            if re_y2_n1 >= h_crop: re_y2_n1 = h_crop
                            ...
                            str3 =list3[:-4] + '.jpg' + ' ' + classname[int(classid[4])] + ' ' + str(re_x1_n1) + ' ' + str(re_y1_n1) + ' ' + str(re_x2_n1) + ' ' + str(re_y2_n1) + '\n'
                            re_image_path = image_new+list3[:-4] + '.jpg'
                            #cv2.rectangle(re_imgcrop, (re_x1_n1, re_y1_n1), (re_x2_n1, re_y2_n1), (0, 255, 0), 1)
                    ...
                    for t in range(sumcount_0+1,sumcount_0+count_list[j-1]+1):
                        ...
                        for s in range(1,j):
                            ...
                            if IOU >= 0.7:
                                ...
                                w_new_33 = str2x2 - str2x1
                                h_new_33 = str2y2 - str2y1
                                re_x1_n2 = int(str2x1 - context_1 * w_new_33)
                                re_y1_n2 = int(str2y1 - context_2* h_new_33)
                                re_x2_n2 = int(str2x2 + context_1* w_new_33)
                                re_y2_n2 = int(str2y2 + context_2* h_new_33)
                                if re_x1_n2 <= 0: re_x1_n2 = 0
                                if re_x2_n2 >= w_3: re_x2_n2 = w_3
                                if re_y1_n2 <= 0: re_y1_n2 = 0
                                if re_y2_n2 >= h_3: re_y2_n2 = h_3
                                ...
                                #cv2.rectangle(imgcrop_3, (re_x1_n2, re_y1_n2), (re_x2_n2, re_y2_n2), (0, 255, 0), 1)
                                cv2.imwrite(image_new + list2[:-4]+'.jpg', imgcrop_3)
                                str2 =list2[:-4] + '.jpg' + ' ' + classname[int(labelxy[(s - 1) * 5 + 4])] + ' ' + str(re_x1_n2) + ' ' + str(re_y1_n2) + ' ' + str(re_x2_n2) + ' ' + str(re_y2_n2) + '\n'
