...
def rename(inputimagePath,inputxmlPath,outimagePath,outxmlPath):
    ...
    for item in filelist:
        if item.endswith('.jpg'):  # 初始的图片的格式为bmp格式的（或者源文件是png格式及其他格式，后面的转换格式就可以调整为自己需要的格式即可）
            src = os.path.join(os.path.abspath(inputimagePath), item)  #
            dst = os.path.join(os.path.abspath(outimagePath), '0' + format(str(i), '0>5s') + '.jpg')    # 这种情况下的命名格式为0000.bmp形式，可以自主定义想要的格式
            try:
                os.rename(src, dst)
...
if __name__ == '__main__':
    inputimagePath = '/home/cowa/lhb/py-R-FCN/data/VOCdevkit/baidu/baidu_images/' 
    inputxmlPath='/home/cowa/lhb/py-R-FCN/data/VOCdevkit/baidu/baidu_labels/'
    outimagePath = '/home/cowa/lhb/py-R-FCN/data/VOCdevkit/baidu/images/' 
    outxmlPath='/home/cowa/lhb/py-R-FCN/data/VOCdevkit/baidu/labels/'
