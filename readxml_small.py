...
if __name__=='__main__':
	if len(sys.argv) < 7:
		print sys.argv[0], 'path-to-process'
		sys.exit(-1)
	image_root=sys.argv[1]
	label_root=sys.argv[2]
	image_new=sys.argv[3]
	label_new=sys.argv[4]
	image_see=sys.argv[5]
	label_new2=sys.argv[6]
	...
	folder1 = os.path.exists(label_new)
	if not folder1:
		os.makedirs(label_new)
	folder2 = os.path.exists(image_see)
	if not folder2:
		os.makedirs(image_see)
	folder3 = os.path.exists(label_new2)
	if not folder3:
		os.makedirs(label_new2)       
	for file_name in files: 
		if file_name.endswith('xml'):
			...
			j=0
			name_num=0
			new_path1=label_root+file_name
			print(label_root+file_name)
			...
			for i in xrange(len1):
				...
				randw = random.randint(500, 600)
				randh = random.randint(100, 300)
				...
				if IOU > 0.6:
					...
					w_new = x2_new - x1_new
					h_new = y2_new - y1_new
					new_labeltxt = label_new+imgname[:-4] + '-' + str(j) + '.txt'
					new_labeltxt_xml1=label_new2+imgname[:-4] + '-' + str(j) + '.txt'
					name_num = name_num + 1
					...
					images_name_path=image_see+imgname[:-4]+'-' + str(j)+'.jpg'
				...
				if j>=2:
					for r in range(1,j):
						...
						if IOU >= 0.6:
							...
							re_image_path = image_see + list3[:-4]+ '.jpg'
					...
					for s in range(1, j):
						...
						if IOU >= 0.6:
							...
							images_name3=image_see + list2[:-4]+ '.jpg'
